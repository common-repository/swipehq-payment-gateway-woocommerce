...
add_filter('plugin_action_links', 'swipe_action_links', 10, 2 );

function swipe_action_links( $links, $pluginLink ){
	if($pluginLink != 'swipehq-payment-gateway-woocommerce/index.php') return $links;
	$plugin_links = array(
			'<a href="' . admin_url( 'admin.php?page=woocommerce_settings&tab=payment_gateways&section=WC_SwipeHQ_Gateway' ) . '">' . __( 'Settings', 'Optimizer' ) . '</a>',
	);
	...
	class WC_SwipeHQ_Gateway extends WC_Payment_Gateway {
		...
		function getAcceptedCurrencies(){
			...
			$params = array(
					...
					'api_key' => $this->working_key // TODO: rename this to api_key
 			);
		...
		public function admin_options(){
			echo '<h3>'.__('Swipe Checkout', 'mrova').'</h3>';
		...
		/**
		 *  There are no payment fields for Swipe, but we want to show the description if set.
		 **/
		...
		function receipt_page($order){
			...
			$params = array (
					...
					'td_amount'             => $order->order_custom_fields['_order_total'][0],
